/* jquery.carousel.js v2.5.0 | (c) n33 | n33.co | MIT licensed */

(function($) {

	// Disables selection
		$.fn.carousel_disableSelection = function() { return $(this).css('user-select', 'none').css('-khtml-user-select', 'none').css('-moz-user-select', 'none').css('-o-user-select', 'none').css('-webkit-user-select', 'none'); }

	// carousel prototype method
		$.fn.carousel = function(options) {
			...
			// Settings
				var settings = $.extend({
					...
					overlayClass:					'carousel-overlay',			// Overlay class
					...
					popupClass:						'carousel-popup',			// Popup class
			...
			// Main
				...
				$nav_next
					.on('click', function() {
						$popup.trigger('carousel_next');
					});

				$nav_previous
					.on('click', function() {
						$popup.trigger('carousel_previous');
					});
				...
				$popup
					...
					.on('carousel_next', function() {
						...
						$popup.trigger('carousel_switch', [x]);
					
					})
					.on('carousel_previous', function() {
					...
					.on('carousel_open', function(e, index) {
						...
						$overlay
							.fadeTo(settings.fadeSpeed, 1.0, function() {
								$popup.trigger('carousel_switch', [index, true]);
							});

					})
					.on('carousel_switch', function(e, index, ignoreLock) {
				...
				$this.find(settings.selector).each(function(index) {
					...
					var x, tmp, a = $(this), i = a.find('img'), data = a.data('carousel');
					...
					// Create object (based on type)
						...
						switch (x.type) {
							...
							case 'ajax':
								x.object = $('<div class="carousel-ajax"></div>');
								x.object
									.on('click', function(e) { e.stopPropagation(); })
									.css('cursor', 'auto')
									.css('overflow', 'auto');
					...
					if (x.type != 'ignore')
						a
							...
							.on('click', function(e) {
								...
								$popup.trigger('carousel_open', [index]);
